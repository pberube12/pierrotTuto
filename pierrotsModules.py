...
def renameAvid(originalPath):
    ...
    Ceci est la fonction qui renomme le quicktime original en rajoutant _AVID
        ...
        listeQtRecherche : La liste qu'on veut renvoyer des qt renommÃ©
...
def renameZip(originalPath):
    ...
    Ceci est la fonction qui renomme le quicktime original en ramplacant le .mov par .zip
        listOriginalPath : c'est la liste(path) des quicktimes qu'on veut modifier
        listeZipRecherche : La liste qu'on veut renvoyer des qt renommÃ©
        filename : filename sans l'extension
        ext : l'extension split 
...
def creationFolder(folder):
    """
    Ceci est une fonction qui fait un nouveau folder au path demandÃ©
        destPath : l'argument passÃ© est le path ou l'on veut crÃ©er le nouveau folder
    """
...
def renameDnx(path):
    ...
    for file in qt:
        if file.startswith("."):
            continue
        qtSplitPoint = file.split(".")
        qtSplitUnder = file.split("_vfx")
        ext = qtSplitPoint[-1]
        filename = qtSplitUnder[0]
        fileDnx = (filename + "." + ext)
        listeQtDnx.append(fileDnx)
    return listeQtDnx

def listingImage(files):
    for listes in newListe:
        listePath = os.path.join(path, listes)
        if os.path.isdir(listePath):
            continue
        fichierDict = {}
        #enlever le .ds_store
        if listes.startswith("."):
            continue
        #fichier séparé par les points
        fileSplit = listes.split(".")
        ext = fileSplit[-1]
